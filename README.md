...
![Parametric sustainable bicycle concept](assets/preview.svg)
...
Outputs are written under `build/`; the checked-in preview is `assets/preview.svg`. Existing nonempty STL outputs are skipped when `RESUME=1`, and each part has an independent timeout and log under `build/logs/`.
