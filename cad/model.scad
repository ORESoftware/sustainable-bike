...
assert(DESIGN_YEAR == 2026 || DESIGN_YEAR == 2027, "unsupported design year");
...
assert(GEAR_COUNT == len(INTERNAL_GEAR_RATIOS), "gear count disagrees with generated ratios");
assert(GEAR_COUNT >= 2, "automatic internal hub requires at least two ratios");
assert(DESIGN_YEAR != 2027 || GEAR_COUNT >= 3, "2027 profile requires three automatic ratios");
...
assert(ACTIVE_THERMAL_DERATING == true, "active thermal derating is required");
assert(REDUNDANT_WHEEL_SPEED_SENSOR == true, "redundant wheel-speed sensing is required");
...
module complete_assembly() {
  ...
  if (SHOW_LABELS) {
    translate([WHEELBASE * 0.50, -78, wheel_radius + 8])
      rotate([90, 0, 0])
        label_plate(str("SB ", DESIGN_YEAR, " / ", GEAR_COUNT, " AUTO RATIOS"), 245, 28, 3);
  }
}
